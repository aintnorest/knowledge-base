...
- `log.md` is the append-only ingest history.
...
Identify each source using the strongest available evidence, in this order:

1. DOI or another canonical source identifier
2. arXiv identifier, treating revisions of the same base arXiv ID as the same
   source unless the task explicitly requests a revision update
3. Normalized title and authors
4. Exact file checksum
5. Filename as supporting evidence only

Compare the identity with:

- dossier frontmatter and content
- `archive/index.md`
- `log.md`
- existing archive filenames and file metadata

A match in any one of these locations makes the candidate ineligible. Do not
re-ingest it because another part of its earlier ingest is missing or
inconsistent. Report that inconsistency without repairing it unless the task
...
8. Append a dated ingest entry to `log.md`. Do not rewrite prior entries.
